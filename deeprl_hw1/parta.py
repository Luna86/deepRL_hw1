...
def main():
    ...
    policy, value_func, policy_iter_idx, value_iter_idx = policy_iteration(env, gamma, max_iterations=int(1e3),
    tol=tol)
    time_elapsed = time.time() - start_time
    print('Time for execution of policy iteration: {0}'.format(time_elapsed))
    print('Number of policy improvement steps: {0}'.format(policy_iter_idx))
    print('Number of policy evaluation steps: {0}'.format(value_iter_idx))
    #print('Value function by policy iteration: {0}'.format(value_func))
    print('Value function by policy iteration: ')
    for i in range(8):
        print(" ".join([str(round(x,4)) for x in value_func[i*8:(i+1)*8]]) + ";")
    ...
    total_reward, num_steps = run_optimal_policy(env, gamma, policy)
